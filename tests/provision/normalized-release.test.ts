...
async function createProductionRelease(
...
): Promise<{ release: NormalizedInvitationRelease; cleanup: () => void }> {
	...
	for (const spec of ROMINA_ASSET_SPECS) {
		const bytes = spec.relativePath.endsWith('.webp')
			? await sharp(sampleJpeg).webp().toBuffer()
			: sampleJpeg;
...
describe('normalized managed release semantics & roundtrip parity', () => {
	...
	it('proves end-to-end Local/Package materialization roundtrip parity for Romina invitation', async () => {
		...
		try {
			...
			const localMaterializedDraft = materializeAssetReferences(
			...
			const packageMaterializedDraft = materializeAssetReferences(
				serializedPkg.draft.content,
				targetAssetUuidMap,
			) as Record<string, unknown>;
			...
			function normalizeEnv(obj: unknown): unknown {
				...
				if (obj !== null && typeof obj === 'object') {
					...
					return Object.fromEntries(
						Object.entries(rec).map(([k, v]) => [k, normalizeEnv(v)]),
					);
				}
				return obj;
			}
			...
			expect(normalizeEnv(localMaterializedDraft)).toEqual(
				normalizeEnv(packageMaterializedDraft),
			);
			expect(release.draftContent).not.toHaveProperty('theme');
			expect(release.publishedProjection).toHaveProperty('theme');
			...
			expect(serializedPkg.assets.map((a) => a.key).sort()).toEqual(
				ROMINA_ASSET_SPECS.map((s) => s.key).sort(),
			);
