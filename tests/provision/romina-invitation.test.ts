...
jest.mock('node:fs', () => {
  ...
  return {
    ...
    readFileSync: jest.fn().mockImplementation((p: any, enc: any) => actual.readFileSync(p, enc)),
...
async function importProvisioner(): Promise<typeof import('../../scripts/provision/romina-invitation')> {
...
function createMockClient(): { client: any; builder: Record<string, jest.Mock> } {
  const builder: Record<string, jest.Mock> = {
    ...
    is: jest.fn<() => Promise<{ data: unknown; error: null }>>()
      .mockResolvedValue({ data: [], error: null }),
    order: jest.fn().mockReturnThis(),
    limit: jest.fn().mockReturnThis(),
    range: jest.fn().mockReturnThis(),
    maybeSingle: jest.fn<() => Promise<{ data: unknown; error: null }>>()
      .mockResolvedValue({ data: null, error: null }),
    single: jest.fn<() => Promise<{ data: unknown; error: null }>>()
      .mockResolvedValue({ data: { id: 'mock-id' }, error: null }),
  };
  const stub = {
    from: jest.fn(() => builder),
    rpc: jest.fn<() => Promise<{ data: unknown; error: null }>>()
...
describe('provision/romina-invitation.ts', () => {
  ...
  describe('npm script registration', () => {
    it('registers invitation:prod:provision in package.json', () => {
      ...
      expect(pkg.scripts['invitation:prod:provision']).toMatch(/tsx scripts\/provision\/romina-invitation-cli\.ts/);
    });
  });
  ...
  describe('parseArgs', () => {
    ...
    it('exits when both --dry-run and --apply are provided', async () => {
      const mod = await importProvisioner();
      process.argv = ['node', 'script.ts', '--dry-run', '--apply', '--owner-user-id', VALID_UUID, '--source-dir', '/tmp'];
      assertExits(() => mod.parseArgs());
    });
    ...
    it('exits when source dir does not exist', async () => {
      ...
      const mod = await importProvisioner();
      process.argv = ['node', 'script.ts', '--dry-run', '--owner-user-id', VALID_UUID, '--source-dir', '/nonexistent'];
      assertExits(() => mod.parseArgs());
    });
  });
  ...
  describe('redactSecrets', () => {
    it('redacts service role key from error messages', async () => {
      const mod = await importProvisioner();
      const msg = 'Error with SUPABASE_SERVICE_ROLE_KEY=eyJhbGciOiJIUzI1NiJ9.eyJyb2xlIjoic2VydmljZV9yb2xlIn0.fake';
  ...
  describe('buildAssetActions', () => {
    ...
    it('classifies identical asset as reuse', async () => {
      ...
      existing.set('Hero', {
        id: 'asset-1', displayName: 'Hero', storagePath: 'path/hero.webp',
        fileSize: 50000, width: 1200, height: 800, imageHash: null,
      });
  ...
  describe('ensureInvitation', () => {
    it('returns abort when owned by different user', async () => {
      ...
      builder.is = jest.fn<() => Promise<{ data: unknown; error: null }>>()
        .mockResolvedValue({
    ...
    it('creates a new invitation when none exists (apply mode)', async () => {
      ...
      builder.is = jest.fn<() => Promise<{ data: unknown; error: null }>>()
        .mockResolvedValue({ data: [], error: null });
      builder.single = jest.fn<() => Promise<{ data: unknown; error: null }>>()
  ...
  describe('findExistingAssets', () => {
    it('aborts on duplicate active rows', async () => {
      ...
      builder.is = jest.fn<() => Promise<{ data: unknown; error: null }>>()
        .mockResolvedValue({
          data: [
            { id: 'a1', display_name: 'Hero', storage_path: 'p1', file_size: 100, width: 100, height: 100 },
            { id: 'a2', display_name: 'Hero', storage_path: 'p2', file_size: 100, width: 100, height: 100 },
          ],
          error: null,
        });
      await expect(mod.findExistingAssets(client, 'inv-1')).rejects.toThrow('process.exit called');
      expect(exitSpy).toHaveBeenCalledWith(1);
    });
  });
  ...
  describe('findInvitation', () => {
    it('aborts with multiple active invitations', async () => {
      ...
      await expect(mod.findInvitation(client, 'romina-rios-chaparro', 'xv')).rejects.toThrow('process.exit called');
      expect(exitSpy).toHaveBeenCalledWith(1);
    });
  });
  ...
  describe('publishInvitation', () => {
    it('calls publish_invitation_atomic RPC', async () => {
      ...
      expect(client.rpc).toHaveBeenCalledWith('publish_invitation_atomic', expect.objectContaining({
    ...
    it('returns version from publishedContent', async () => {
      const mod = await importProvisioner();
      const { client } = createMockClient();
      client.rpc = jest.fn<() => Promise<{ data: unknown; error: null }>>()
        .mockResolvedValue({ data: { publishedContent: { version: 3 } }, error: null });
      const result = await mod.publishInvitation(client, 'inv-1', 'draft-1', '2026-01-01T00:00:00Z', {});
