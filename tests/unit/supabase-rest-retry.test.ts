...
describe('JWT future retry', () => {
	it(
		'retries a matching PGRST303 GET and succeeds on second attempt',
		async () => {
			mockFetchSequence(
	...
	it(
		'stops retrying after max attempts and throws the original SupabaseHttpError',
		async () => {
			mockFetchSequence(
				errorResponse(401, JWT_FUTURE_BODY),
				errorResponse(401, JWT_FUTURE_BODY),
			...
			expect((error as SupabaseHttpError).body).toContain(
...
describe('unrelated PGRST303 messages', () => {
	it('does not retry PGRST303 with a different message', async () => {
		...
		await expect(supabaseRestRequest(authGetOptions)).rejects.toThrow(
			SupabaseHttpError,
		);
...
describe('log diagnostic safety', () => {
	it(
		'never logs the JWT or bearer token in warn output',
		async () => {
			mockFetchSequence(
				errorResponse(401, JWT_FUTURE_BODY),
				okResponse(JSON.stringify([{ id: 1 }])),
...
describe('no-regression success path', () => {
	it('returns data on first success without retry', async () => {
		mockFetchSequence(
			okResponse(JSON.stringify([{ id: 1, name: 'event' }])),
		);
