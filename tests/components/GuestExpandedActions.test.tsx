...
describe('guest dashboard expanded actions', () => {
	...
	it('shows Marcar como enviado when guest is not yet sent', () => {
		render(
		...
		expect(screen.queryByText('Marcar como no enviado')).not.toBeInTheDocument();
	});

	it('shows Marcar como no enviado when guest is already sent', () => {
		render(
			<GuestExpandedActions
		...
		expect(screen.getByText('Marcar como no enviado')).toBeInTheDocument();
		expect(screen.queryByText('Marcar como enviado')).not.toBeInTheDocument();
	});

	it('first click on Marcar como enviado does not call onMarkShared', () => {
		render(
			<GuestExpandedActions
				guestName={sampleGuest.fullName}
				inviteUrl="https://example.com/invite/1"
	...
	it('second click on Confirmar envío calls onMarkShared', async () => {
		render(
			<GuestExpandedActions
				guestName={sampleGuest.fullName}
				inviteUrl="https://example.com/invite/1"
		...
		fireEvent.click(screen.getByText('Marcar como enviado'));
		fireEvent.click(screen.getByText('Confirmar envío'));
		await waitFor(() => {
			expect(onMarkShared).toHaveBeenCalledTimes(1);
		});
	});

	it('first click on Marcar como no enviado does not call onRevertShared', () => {
		render(
			<GuestExpandedActions
				guestName={sentGuest.fullName}
				inviteUrl="https://example.com/invite/2"
		...
		fireEvent.click(screen.getByText('Marcar como no enviado'));
		expect(onRevertShared).not.toHaveBeenCalled();
		expect(screen.getByText('Confirmar cambio')).toBeInTheDocument();
	});

	it('second click on Confirmar cambio calls onRevertShared', async () => {
		render(
			<GuestExpandedActions
				guestName={sentGuest.fullName}
				inviteUrl="https://example.com/invite/2"
