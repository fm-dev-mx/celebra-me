...
describe('CanonicalStatusPanel', () => {
	...
	it('prioriza salud global, acciones visibles y una sola jerarquía H2 del panel', () => {
		...
		expect(screen.getByRole('heading', { level: 2, name: 'Estado operacional' })).toBeInTheDocument();
	...
	it('mantiene No aplica neutral y separa publicación sin acción', () => {
		render(<CanonicalStatusPanel initialView={buildCanonicalStatusViewFixture({ promotions: [], inSyncCount: 5, inSyncSlugs: ['a', 'b', 'c', 'd', 'e'] })} />);
	...
	it('deja detalles avanzados colapsados y sin autoridad operativa', () => {
		render(<CanonicalStatusPanel initialView={buildCanonicalStatusViewFixture({ diagnostics: [{ code: 'MANAGED_DRIFT', domain: 'content', evidence: 'LIVE', slug: 'victoria-y-roberto', environment: 'preview', cause: 'Live managed content differs from the canonical definition.', affectedFieldCount: 2, affectedSectionCount: 1, semanticPaths: ['hero.title'] }] })} />);
		const summary = screen.getByText('Diagnóstico avanzado (1)');
		expect(summary.closest('details')).not.toHaveAttribute('open');
		expect(screen.getByText('Live managed content differs from the canonical definition.')).toBeInTheDocument();
	...
	it('muestra autorización Owner ausente como bloqueo sin inventar comando', () => {
		const base = buildCanonicalStatusViewFixture();
		render(<CanonicalStatusPanel initialView={buildCanonicalStatusViewFixture({ environments: { ...base.environments, production: { ...base.environments.production, authorizationIntegrity: 'MISSING', authorizationMissingVersions: ['20260807120000'] } } })} />);
		expect(screen.getByText(/Autorización · Producción/)).toBeInTheDocument();
		expect(screen.getAllByText(/No se pueden registrar applies históricos/).length).toBeGreaterThan(0);
	...
	it('distingue caché de evidencia live y conserva historial colapsado', () => {
		render(<CanonicalStatusPanel initialView={buildCanonicalStatusViewFixture({ freshnessMeta: { status: 'CACHED', lastVerifiedAt: '2026-08-12T22:11:54.000Z' }, recentMigrations: [{ version: '20260806120000', name: 'base.sql', presence: { local: 'APPLIED', preview: 'UNVERIFIED', production: 'NOT_APPLIED' }, verifiedAt: { local: '2026-08-12T22:11:54.000Z', preview: null, production: '2026-08-12T22:11:54.000Z' } }] })} />);
		...
		expect(screen.getByText(/Historial de migraciones recientes/).closest('details')).not.toHaveAttribute('open');
	});

	it('conserva la vista durable ante fallo de refresh y permite reintentar', async () => {
		const user = userEvent.setup();
		mockGet.mockResolvedValueOnce({ ok: false, status: 504, code: 'service_unavailable', message: 'La consulta de estado excedió el tiempo límite.' }).mockResolvedValueOnce({ ok: true, status: 200, data: buildCanonicalStatusViewFixture({ evidence: 'CACHED' }) });
		render(<CanonicalStatusPanel initialView={buildCanonicalStatusViewFixture()} />);
	...
	it('evita doble refresh y mantiene controles etiquetados dentro del alcance', async () => {
		...
		mockGet.mockImplementationOnce(() => new Promise<never>((resolve) => { resolveRefresh = resolve; }));
		render(<CanonicalStatusPanel initialView={buildCanonicalStatusViewFixture()} />);
		...
		resolveRefresh?.({ ok: true, status: 200, data: buildCanonicalStatusViewFixture() } as never);
