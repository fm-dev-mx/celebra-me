	...
	it('derives ordered section inventory and computes bounding box from top to bottom section', async () => {
		const mockPage = {
			evaluate: async () => ({
				...
				rendered: 3,
				sections: [
					{ id: 'hero', label: 'Hero', order: 1, selector: '[data-screenshot-section="hero"]', bounds: { x: 0, y: 0, width: 390, height: 800 }, isVisible: true },
					{ id: 'location', label: 'Location', order: 2, selector: '[data-screenshot-section="location"]', bounds: { x: 0, y: 800, width: 390, height: 600 }, isVisible: true },
					{ id: 'thankYou', label: 'Thank You', order: 3, selector: '[data-screenshot-section="thankYou"]', bounds: { x: 0, y: 1400, width: 390, height: 400 }, isVisible: true },
				],
				duplicates: [],
				missing: [],
				topY: 0,
		...
		const inventory = await deriveSectionInventory(mockPage as unknown as import('playwright').Page);
	...
	it('deduplicates repeated or nested markers by stable section identity', async () => {
		const mockPageWithDuplicates = {
			evaluate: async () => ({
				...
				sections: [
					{ id: 'hero', label: 'Hero', order: 1, selector: '[data-screenshot-section="hero"]', bounds: { x: 0, y: 0, width: 390, height: 800 }, isVisible: true },
					{ id: 'gallery', label: 'Gallery', order: 2, selector: '[data-screenshot-section="gallery"]', bounds: { x: 0, y: 800, width: 390, height: 1200 }, isVisible: true },
				],
				duplicates: ['gallery'],
				missing: [],
				topY: 0,
		...
		const inventory = await deriveSectionInventory(mockPageWithDuplicates as unknown as import('playwright').Page);
		expect(inventory.duplicates).toContain('gallery');
		expect(inventory.sections.length).toBe(2);
	});

	it('plans all section tasks dynamically in order', async () => {
		const mockPage = {
			locator: () => ({
				count: async () => 1,
			...
			evaluate: async (fn: unknown) => {
				if (typeof fn === 'function' && fn.toString().includes('ds-editorial-cover')) {
					return false;
				}
				return {
					expected: 2,
					...
					sections: [
						{ id: 'hero', label: 'Hero', order: 1, selector: '[data-screenshot-section="hero"]', bounds: { x: 0, y: 0, width: 390, height: 800 }, isVisible: true },
						{ id: 'quote', label: 'Quote', order: 2, selector: '[data-screenshot-section="quote"]', bounds: { x: 0, y: 800, width: 390, height: 300 }, isVisible: true },
					],
					duplicates: [],
					missing: [],
					topY: 0,
		...
		const tasks = await resolveCapturePlan(mockPage as unknown as import('playwright').Page, job);
