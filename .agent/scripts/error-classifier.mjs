...
function parseLog(log) {
	...
	if (log.includes('FAIL ') && (log.includes('tests/') || log.includes('e2e/'))) {
        ...
        if (srcLineMatch) {
            ...
            const lineMatch = log.match(/at\s+.*?\s+\(?([a-zA-Z]:\\[\w.\-\\]+|\/[\w.\-\/]+):(\d+):(\d+)\)?/);
            if (lineMatch) {
        ...
        if (report.message && report.message.match(/is not defined|Cannot find name|Missing semicolon/i)) {
	...
	if (log.includes('astro build') || log.includes('AstroError') || log.includes('Hydration')) {
        ...
        report.snippet = extractSnippet(lines, log.indexOf('AstroError') > -1 ? log.indexOf('AstroError') : 0);
		return report;
	}
...
if (process.argv[2]) {
	try {
		...
		console.error(JSON.stringify({ ...DEFAULT_REPORT, message: `Failed to read: ${err.message}` }));
		process.exit(1);
	}
} else if (!process.stdin.isTTY) {
	process.stdin.setEncoding('utf-8');
	process.stdin.on('data', chunk => { input += chunk; });
	process.stdin.on('end', () => { parseAndPrint(input); });
} else {
    console.error(JSON.stringify({ ...DEFAULT_REPORT, message: 'No input provided via stdin or argument.' }));
    process.exit(1);
}
