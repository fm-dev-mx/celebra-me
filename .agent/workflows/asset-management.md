...
This workflow guides the transition of event image infrastructure to a structured **Universal Asset
Registry/System**. It handles curation, semantic renaming, registry updates, and verification of
event assets.

## 1. Context & Discovery

1. **Source of Truth Check**
    - Verify the contents of `src/assets/images/events/[event-slug]/`.
...
**Action**: Rename files in `src/assets/images/events/[event-slug]/`.
...
3. **Universal Asset Registry** (`src/lib/assets/AssetRegistry.ts`)
    - Register the event assets via the canonical registry so app-level imports stay consistent.

## 4. Content Integration

Update the Event Content Collection (`src/content/events/[event-slug].json`) to reference the new
architecture.

- **Deprecation**: Remove raw string paths (e.g., `"../../assets/..."`).
- **Adoption**: Use the registry keys or imports defined in the `[slug].astro` page logic (or
...
3. **Clean Up**
    - Once verified, delete any legacy raw images that are no longer referenced to keep the repo
      clean.

## 📝 Related Workflows

- [/universal-asset-system](./universal-asset-system.md): The overarching orchestration for all
  assets.
