...
| Historical direct `db:prod:patch --apply` path (retired) | Specialized DML (manifest SQL) | Replaced by composite `prod:apply --patch` | `PROD_DB_URL` + `SUPABASE_URL` same project + `--owner-user-id` | Current owner path rebinds the reviewed plan | Lint + required manifest; direct `--dry-run` does not connect | None | Current path requires a critical backup | `db:contract:verify` after SQL | Plan-bound workflow | Historical split-brain risk is removed from the direct entrypoint |
