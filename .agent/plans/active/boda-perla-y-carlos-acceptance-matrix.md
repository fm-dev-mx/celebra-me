---
title: Boda Perla y Carlos — Acceptance Matrix (Closure)
status: active
created: 2026-08-01
updated: 2026-08-01
related_docs:
...
# Acceptance matrix — `boda-perla-y-carlos`

Comparison baseline: local managed published content (version 5, IN_SYNC) + jewelry-box-wedding
shared structure. Production route `/boda/boda-perla-y-carlos` returns **404** (not promoted); local
...
| Vertical rhythm / section continuity | A               | Shared `svh` mins retained; Perla densifies quote/countdown/thank-you          |
...
| Family placeholders                  | A               | Soft `"Por confirmar"` (OD5 / B1 keep)                                         |
...
| ---------------------------------------------------------- | --------------- | ------------------------------------- |
| Kill hero gradient/specular clip (invisible desktop names) | Lane A          | `boda-perla-y-carlos.scss`            |
| Readiness honors `venues[]` + `googleMapsUrl`              | Shared          | `invitation-readiness.ts`             |
| Payload test locks solid title fill                        | Invitation test | `boda-perla-y-carlos-payload.test.ts` |
...
| Artifacts                                  | `output/playwright/boda-perla-y-carlos/*.png`                                     |
...
- Full `pnpm ci` / `pnpm lint` may still fail on a pre-existing `ObservabilityPanel.tsx` complexity
  lint outside this invitation scope. Perla-scoped gates (`validate:changed`, type-check after mock
