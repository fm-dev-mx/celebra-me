...
| RSVP | **Keep `standard`**. Cheapness is hero/thank-you/PA/gallery, not the RSVP renderer. `formal-pass` + `formal-register` pairing is a residual continuity risk, not a Phase 2 variant change. |
...
| Leslie profile | **Do not create** `src/styles/invitation-profiles/leslie-perez.scss`. `visualProfileId: 'leslie-perez'` may remain; missing file is a no-op. |
