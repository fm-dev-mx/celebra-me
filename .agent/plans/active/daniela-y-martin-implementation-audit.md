---
title: Boda Daniela y Martín — Implementation Audit (Goal 1)
status: active
created: 2026-08-01
updated: 2026-08-01
related_docs:
  - docs/invitations/daniela-y-martin.md
  - .agent/plans/active/daniela-y-martin-audit-backlog.md
...
# Goal 1 — Implementation audit: `daniela-y-martin`
...
| ---- | --------- | ------------- |
| **Provision / content SoT** | Invitation-specific | `scripts/provision/invitations/daniela-y-martin.ts`, registry `scripts/provision/invitations/registry.ts` |
| **Prep facts** | Docs (not runtime) | `docs/invitations/daniela-y-martin.md` |
| **Lane A skin** | Invitation-specific SCSS | `src/styles/invitation-profiles/daniela-y-martin.scss` |
| **Assets** | Invitation-specific | `src/assets/invitations/daniela-y-martin/{hero-source,hero-mobile-source,gallery-01-source}.jpg` |
...
| **Location / dress / civil** | Shared markup; Daniela content | `EventLocation.astro`, `VenueCard.astro`, `location-helper.ts`, `location-policy.ts`, `invitation/_event-location.scss`; indications owned by provision payload |
| **Family** | Shared + Daniela content | `Family.astro`, `invitation/_family.scss`; groups/labels in provision |
...
```
Provision buildPublishedContent (daniela-y-martin.ts)
...
| ----- | ------------- | ------------------- | ---- |
| Timing | `DANIELA_EVENT` (`localDateTime`, `startsAtUtc`, `timeZone`, `heroDate`) | Countdown via `resolveCountdownTarget`; Hero date via UTC `toLocaleDateString('es-MX')`; reveal card via `formatCardDate`; envelope teaser **explicit** `teaserDetails` | Multiple formatters; location uses **hand-written** Spanish date/time strings |
...
- Daniela sets **identical** strings for `microcopy` and `tooltipText` → two “Abrir invitación”
...
1. **Lane A dual-scope token remap** — `.event--daniela-y-martin.theme-preset--jewelry-box-wedding`
   with local `--daniela-*` + RGB channels (do not mutate shared preset).
...
- **Date/time:** ISO in `DANIELA_EVENT` + Spanish strings in location venues + thankYou.date +
...
- Wedding section bundle does **not** load `themes/sections/{hero,countdown,rsvp,thank-you}/_jewelry-box.scss`
  (XV `jewelry-box` bundle does). Daniela rides base invitation SCSS + preset tokens + Lane A.
...
| File | Why |
| ---- | --- |
| `scripts/provision/invitations/daniela-y-martin.ts` | Reveal CTA copy; hero venue/time representation; civil/family/gallery/RSVP content corrections within existing schema; asset path consolidate |
| `src/styles/invitation-profiles/daniela-y-martin.scss` | Fix RSVP dark-band vs cream labels; spacing tweaks that must stay Lane A; thank-you/location token fixes without Lane B |
| `tests/content/daniela-y-martin-payload.test.ts` | Lock corrected content contracts |
| `docs/invitations/daniela-y-martin.md` | Record implementation notes only if facts/decisions change |
...
| Item | Action |
| ---- | ------ |
| `src/assets/invitations/daniela-y-martin/hero-mobile-source.jpg` | Delete after pointing `hero-mobile.relativePath` at `hero-source.jpg` (keep distinct focals) |
...
| -------- | ------ | ------------- |
| `tests/content/daniela-y-martin-payload.test.ts` | Exists — schema, sectionOrder, Por confirmar, gallery length 1, rsvp modes | Extend assertions for corrected teaser/CTA fields, civil indication, family policy, asset path |
...
1. **Content corrections in provision** (no CSS yet)
   ...
   - Hero metadata: express ceremony-first for hero without premature shared refactor — prefer
     ...
     location Spanish strings with `DANIELA_EVENT` times.
...
3. **Lane A SCSS** — paint RSVP (and PA if needed) dark olive band so cream labels contrast; fix any
   flagged thank-you/location token issues using existing `--daniela-*` pattern; **do not** change
...
| -------- | ------- | -------- |
| **Canonical formatted wedding metadata** | **No single formatted SSOT.** Raw timing SSOT = `DANIELA_EVENT` / `eventTiming`. Guest-facing Spanish venue strings are provision-authored. Formatters: Hero UTC locale, `formatCardDate`, countdown resolver, explicit `teaserDetails`. Goal 2 must edit provision strings + keep ISO fields coherent — do not invent a new formatter module. | `daniela-y-martin.ts`; `event.ts`; `page-data.ts`; `reveal-card.ts` |
| **Shared vs Daniela visual rules** | **Palette / inheritance reset / RSVP band / thank-you retint → Lane A.** Structure, reveal machine, section components, base `svh` rhythm → shared. Jewelry-box section chrome for hero/rsvp/thank-you is **not currently delivered** on wedding — treating that gap as Lane B. | Profile SCSS; `jewelry-box-wedding.scss` bundle vs `jewelry-box.scss` |
...
| B3 | **Hero ceremony-first via Daniela content vs shared `pickVenueValue`/`pickHeroValue`** | Shared change affects all legacy ceremony+reception invites that prefer reception today. | Prefer Perla-only content shaping unless owner wants Lane B |
