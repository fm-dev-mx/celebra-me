...
Romina’s appearance comes from the shared renderer (`Countdown.astro` + `CountdownTimer.astro`)
plus `_editorial.scss` (`:is([data-variant='editorial'], [data-variant='premiere-floral'])`).
Romina does not set a countdown `variant` or `presentation`; `data-variant` inherits
`premiere-floral`. Renata inherits `editorial`. Both load the same countdown stylesheet.
Romina’s profile tints color only. Renata’s previous profile flattened grid, padding, borders,
and tracking.
...
5. Countdown: remove Daniela-style geometry flattening; keep shared editorial chrome; tint only with Renata olive/cream/coral. Do not copy Romina copy, fonts, or gold.
