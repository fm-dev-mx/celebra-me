...
domain: frontend | backend | content | workflow | quality | meta | growth | qa
version: 1.0.0
when_to_use:
...
Use `workflow` for repeatable orchestration/execution guidance and `quality` for testing or
verification practices. `meta` and `qa` remain valid for existing cross-cutting governance and
review-oriented skills; do not relabel them mechanically.
