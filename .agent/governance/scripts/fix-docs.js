...
for (const f of filesToFix) {
	try {
		...
		text = text.replace(
			/src\/pages\/api\/rsvp\/admin\.ts/g,
			'src/pages/api/dashboard/admin/events.ts',
		);
		text = text.replace(
			/src\/pages\/api\/rsvp\/export\.csv\.ts/g,
			'src/pages/api/dashboard/admin/export.ts',
		);
		text = text.replace(
			/src\/pages\/api\/rsvp\/invitations\.ts/g,
			'src/pages/api/invitacion/rsvp.ts',
		);
		text = text.replace(/src\/lib\/rsvp\/adminAuth\.ts/g, 'src/lib/rsvp/adminProtection.ts');
		text = text.replace(
			/\.agent\/gatekeeper\/policy\.json/g,
			'.agent/governance/config/policy.json',
		);
		text = text.replace(
			/\.agent\/gatekeeper\/baseline\.json/g,
