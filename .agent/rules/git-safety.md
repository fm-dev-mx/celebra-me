...
- otherwise mutate Git state (HEAD, index, branches, tags, history, stash, or unrelated working-tree
  files).
...
Before making file edits or running mutable or environment-affecting commands inside a worktree
lane, agents **must** establish the following state:
