...
const MEDIA_COLUMNS =
	'id,event_key,session_id,object_key,mime_type,size_bytes,duration_seconds,caption,status,created_at,updated_at,accepted_at,rejected_at,deleted_at';
...
export async function completeGuestMemoryItem(input: {
	...
	if (item.status !== 'uploading') return toPublicItem(item);
	if (!canTransitionValentinaMemoriesMedia(item.status, 'validating')) {
	...
	const rows = await supabaseRestRequest<MediaRow[]>({
		...
		useServiceRole: true,
		prefer: 'return=representation',
		body: { status: 'validating', updated_at: new Date().toISOString() },
	});
	if (!rows[0])
		throw new ApiError(503, 'service_unavailable', 'No se pudo confirmar el recuerdo.');
