...
export interface WaxSealProps extends IconProps {
	initials?: string;
	idPrefix?: string;
}
...
/**
 * Organic melted-wax seal renderer.
 *
 * Features an organic melted silhouette, shape-conforming inner cuño with quiet echo,
 * and a recessed monogram plate.
 */
export const WaxOrganicSealIcon: FC<WaxSealProps> = ({
	className,
	size = 24,
	initials,
	idPrefix,
	...
	const gradId = `wax-organic-grad-${rawId}`;
	const embossId = `wax-organic-emboss-${rawId}`;
	const letterId = `wax-organic-letter-${rawId}`;
	const softShadowId = `wax-organic-shadow-${rawId}`;
