...
export interface WaxMedallionSealProps extends IconProps {
	initials?: string;
	idPrefix?: string;
}
...
/**
 * Classic Medallion wax-seal monogram for envelope reveal.
 *
 * Preserves the baseline main implementation featuring concentric circular die rings,
 * specular lighting effects, and classic serif initial scaling.
 */
export const WaxMedallionSealIcon: FC<WaxMedallionSealProps> = ({
	className,
	size = 24,
	initials,
	idPrefix,
	...
	const rawId = idPrefix ? `${idPrefix}-${reactId}` : reactId;
	const gradId = `wax-medallion-grad-${rawId}`;
	const rimGradId = `wax-medallion-rim-${rawId}`;
	const embossId = `wax-medallion-emboss-${rawId}`;
	const letterId = `wax-medallion-letter-${rawId}`;
	const softShadowId = `wax-medallion-shadow-${rawId}`;
			...
			<defs>
				...
				>
					...
					<feDropShadow
						...
						dy="-0.35"
						stdDeviation="0.3"
						floodColor="var(--wax-seal-highlight, #fff8f4)"
						floodOpacity="0.28"
					/>
				</filter>
			</defs>
