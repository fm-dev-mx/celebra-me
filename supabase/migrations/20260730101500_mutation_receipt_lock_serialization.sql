...
-- Serialize invitation-scoped atomic mutations on invitations (FOR UPDATE).
-- Do not row-lock invitation_mutation_operation_receipts: those locks need UPDATE
-- and conflict with the append-only SELECT+INSERT privilege model.

comment on table public.invitation_mutation_operation_receipts is
  'Append-only immutable/idempotency ledger for material invitation mutations. '
  'service_role may SELECT and INSERT only; never UPDATE or DELETE. '
  'Atomic mutation RPCs serialize on the target invitation row (FOR UPDATE), '
  'never on receipt rows. Unique operation_id is the final duplicate defense.';
...
declare
  ...
  v_invitation_locked boolean := false;
...
begin
  ...
  if not v_invitation_locked then
    raise exception using errcode = 'P0001', message = 'editor_invitation_not_found';
  end if;
...
revoke update, delete on table public.invitation_mutation_operation_receipts
