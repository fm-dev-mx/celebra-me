...
-- Previous Vercel instances call this signature. It intentionally cannot write.
create or replace function public.publish_invitation_atomic(
  ...
  select jsonb_build_object('status', 'upgrade_required', 'code', 'publish_upgrade_required');
$function$;
...
comment on function public.publish_invitation_atomic(uuid,uuid,timestamptz,text,text,boolean,jsonb) is 'Phase-one fail-closed compatibility stub. Remove only after updated deployment and cached instances have drained.';
commit;
