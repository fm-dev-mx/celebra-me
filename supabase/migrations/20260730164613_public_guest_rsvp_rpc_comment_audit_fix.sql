...
-- Follow-up: public guest RSVP RPC comment + audit ownership.
-- - p_guest_comment is treated as an absolute value when provided (app owns append formatting).
-- - Null/blank comment leaves the existing guest_comment unchanged.
-- - Remove manual guest_invitation_audit insert; trg_guest_invitations_emit_audit owns status_changed.
...
comment on function public.submit_guest_rsvp_public is
  'Atomic SECURITY DEFINER RPC for public guest RSVP submission. Comment is absolute when provided; audit comes from guest_invitations trigger.';
