...
-- @rollback: restore public.published_invitation_content and matching invitation_content_drafts for slugs ana-sofia-cota-guillen, ayrin-samantha-lerma-castro, and leah-lexa from the pre-apply Production backup

begin;

do $$
declare
  expected_xv text[] := array['ana-sofia-cota-guillen','ayrin-samantha-lerma-castro'];
  expected_slug text;
  published_count int;
...
where p.invitation_project_id = i.id
  ...
  and p.deleted_at is null
  and (
    (i.event_type = 'xv' and i.slug in ('ana-sofia-cota-guillen', 'ayrin-samantha-lerma-castro'))
