...
-- @pair-key: slug
-- @dry-run-query: select 'published' as store, i.slug, p.version from public.invitations i join public.published_invitation_content p on p.invitation_project_id = i.id where i.archived_at is null and p.deleted_at is null and i.event_type = 'xv' and ((i.slug in ('xareni-iyarit','america-johana','ana-sofia-cota-guillen') and (p.content#>>'{itinerary,variant}' is distinct from 'timeline-paper' or p.content#>>'{itinerary,presentation,behavior}' is distinct from 'timeline-paper' or p.content#>>'{gallery,variant}' is distinct from 'index-choreography')) or (i.slug = 'abril-michelle-becerra-rea' and (p.content#>>'{itinerary,variant}' is distinct from 'timeline-paper' or p.content#>>'{itinerary,presentation,behavior}' is distinct from 'timeline-paper'))) union all select 'draft' as store, i.slug, null::integer as version from public.invitations i join public.invitation_content_drafts d on d.invitation_project_id = i.id where i.archived_at is null and d.deleted_at is null and i.event_type = 'xv' and ((i.slug in ('xareni-iyarit','america-johana','ana-sofia-cota-guillen') and (d.content#>>'{itinerary,variant}' is distinct from 'timeline-paper' or d.content#>>'{itinerary,presentation,behavior}' is distinct from 'timeline-paper' or d.content#>>'{gallery,variant}' is distinct from 'index-choreography')) or (i.slug = 'abril-michelle-becerra-rea' and (d.content#>>'{itinerary,variant}' is distinct from 'timeline-paper' or d.content#>>'{itinerary,presentation,behavior}' is distinct from 'timeline-paper')))
