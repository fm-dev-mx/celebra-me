...
export function computePackageHash(
	...
	const sanitized = JSON.parse(JSON.stringify(rest)) as Omit<
	...
	return createHash('sha256')
		.update(
			canonicalize({
				...sanitized,
				assets: [...sanitized.assets].sort((a, b) =>
					(a.key ?? a.storagePath).localeCompare(b.key ?? b.storagePath),
				),
			}),
		)
		.digest('hex');
...
export function serializeInvitationPackage(
	...
	const rawPayload: Omit<InvitationPackageData, 'packageHash'> = {
		...
		projectionHash: release.projectionHash,
		assetManifestHash: release.assetManifestHash,
		definitionCreatedAt: release.definitionCreatedAt,
		...
		invitation: {
			...
			visualProfileId: release.metadata.visualProfileId,
			kind: 'client',
			clientName: release.metadata.clientName,
			hostLoginAlias: release.metadata.hostLoginAlias,
			clientEmail: release.metadata.clientEmail,
			clientWhatsapp: release.metadata.clientWhatsapp,
			photosReceived: release.metadata.photosReceived,
			snapshot: release.metadata.snapshot,
		},
		draft: { status: 'draft', content: release.draftContent },
		publishedContent: { content: release.publishedProjection },
		event: {
			title: release.metadata.title,
			eventType: release.metadata.eventType,
			status: 'published',
...
export async function exportInvitationPackage(
	...
	const packagePath = options.dryRun
		...
		: resolve(
				options.outPath ??
					`.agent/tmp/packages/invitation-${options.slug}-${packageData.packageHash.slice(0, 16)}.json`,
			);
	if (packagePath) {
	...
	return {
		...
		stats: {
			...
			totalBytes: packageData.assets.reduce((sum, asset) => sum + (asset.fileSize ?? 0), 0),
			hasPublishedContent: true,
