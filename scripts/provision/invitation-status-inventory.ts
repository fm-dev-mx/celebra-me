...
export function readFastInvitationInventory(dbUrl: string, definitionSlugs: readonly string[]): FastInventoryResult {
	try {
		...
		const rows = parseRows(result.stdout); const statuses = classifyInvitationInventory(definitionSlugs, rows);
		return { verified: true, rows: rows.map((row) => ({ ...row, status: statuses.get(row.slug) ?? 'LEGACY_REVIEW_REQUIRED' })) };
	} catch {
