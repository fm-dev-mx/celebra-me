export type InvitationInventoryStatus = 'MANAGED' | 'UNAPPLIED_DEFINITION' | 'CANONICAL_REVIEW_REQUIRED' | 'CANONICAL_CONFLICTED' | 'CANONICAL_ASSET_INCOMPLETE' | 'ARCHIVED' | 'DEMO' | 'NOT_PRESENT';
...
export function classifyInvitationInventory(definitionSlugs: readonly string[], rows: readonly InventoryRow[]): Map<string, InvitationInventoryStatus> {
	...
	for (const slug of definitionSlugs) {
		...
		else if (matches.length > 1) result.set(slug, 'CANONICAL_CONFLICTED');
	...
	for (const row of rows) {
		...
		result.set(row.slug, row.archivedAt ? 'ARCHIVED' : row.kind === 'demo' ? 'DEMO' : row.assetComplete === false ? 'CANONICAL_ASSET_INCOMPLETE' : 'CANONICAL_REVIEW_REQUIRED');
	}
	return result;
}
