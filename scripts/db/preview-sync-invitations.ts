...
import { createReport, printReport, writeReportFile, type SyncReport } from './preview-sync-report.ts';
...
const PROD_STORAGE_PATTERN = /https:\/\/ineitkdkyrxqyressllp\.supabase\.co\/storage\/v1\/object\/public\/invitation-assets\//g;
...
function scanContentForProdStorageUrls(prodCtx: ProdContext): {
	...
	for (const table of [...contentTables, ...jsonbTables]) {
		...
		for (const row of rows) {
			...
			const rowContent = applicableCols.map((col) => {
				const val = row[col];
...
function scanContentReferencedStoragePaths(prodCtx: ProdContext): Set<string> {
	...
	const contentTables = ['published_invitation_content', 'invitation_content_drafts'] as const;
	const storagePathPattern = /https:\/\/[a-z0-9]+\.supabase\.co\/storage\/v1\/object\/public\/invitation-assets\/([^\s"',\]]+)/g;

	for (const table of contentTables) {
		const rows = queryTableJson(prodCtx.dbUrl, table);
		for (const row of rows) {
			const contentStr = typeof row.content === 'string'
				? row.content
...
function reconcileEventMemberships(
	...
	if (dryRun) {
		console.info(`   [dry-run] Would ensure owner membership for ${mirroredEventIds.length} mirrored events`);
		return;
	}
	...
	report.created['event_memberships'] = (report.created['event_memberships'] || 0) + mirroredEventIds.length;
}
	...
	return [
		...
		// Phase 2: Scan Production Storage URLs in content
		{
			...
			action: () => {
				...
				if (unregistered.length > 0) {
					console.warn(`   ⚠️  ${unregistered.length} Storage references not registered in invitation_assets:`);
				...
				if (unregistered.length > 0) {
					report.detectedDrift.push(`${unregistered.length} Storage references not in invitation_assets`);
				}
		...
		...MIRROR_TABLES.map((table) => ({
			...
			action: () => {
				...
				const nullifyFkColumns = columns.filter((c) =>
					['submission_id'].includes(c),
				);
				...
				const jsonBColumns = columns.filter((c) =>
					['content', 'snapshot'].includes(c),
				);
		...
		// Phase 4: Remap event ownership
		{
			...
			action: () => {
				...
				reconcileEventMemberships(previewCtx.dbUrl, previewCtx.previewAdminUserId, mirroredEventIds, options.dryRun, report);
		...
		// Phase 6: Storage sync
		{
			...
			action: async () => {
				...
				try {
					...
					console.warn('   ⚠️  PREVIEW_SUPABASE_SERVICE_ROLE_KEY not configured — skipping Storage sync');
					report.detectedDrift.push('Storage sync skipped: missing PREVIEW_SUPABASE_SERVICE_ROLE_KEY');
					return;
				}
		...
		// Phase 8: Post-apply Production URL audit
		{
			...
			action: () => {
				...
				if (options.dryRun) {
					console.info('   (Skipped in dry-run — checked from Production side in Phase 2)');
					return;
				}
				const { rows, total } = scanContentForProdStorageUrls(
					{ ...prodCtx, dbUrl: previewCtx.dbUrl },
				);
				if (total > 0) {
