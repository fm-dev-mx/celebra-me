...
function runDisposableTestCommand(action: string, args: string[] = []): { status: number | null; stdout: string } {
...
function main(): void {
	...
	console.info('ℹ️  Skipped: pgTAP test files cover pending release candidate features (20260717193000+) not present in the baseline cutoff.\n');
	...
	if (previewDbUrl) {
		...
		const previewAuditRes = runCommand('npx', ['tsx', 'scripts/db/audit-db.ts', '--target', 'preview'], {
			throwOnError: false,
		});
...
try {
	...
	console.error('Fatal validation pipeline error:', err instanceof Error ? err.message : String(err));
	process.exit(1);
}
