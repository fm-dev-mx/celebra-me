...
export function evaluateMigrationHistoryParity(
	...
	if (pendingLocal.length > 0) {
		errors.push(`Pending local migrations not applied to remote (${pendingLocal.length}): ${pendingLocal.join(', ')}`);
	}
	if (extraRemote.length > 0) {
		errors.push(`Remote database has extra migrations not found in local workspace (${extraRemote.length}): ${extraRemote.join(', ')}`);
	}
	if (isReordered) {
	...
	const isAligned = pendingLocal.length === 0 && extraRemote.length === 0 && !isReordered && !hasDivergentHistory;
...
function runMigrationsAudit(target: string, dbUrl: string): { remoteVersions: string[]; extraRemoteCount: number } {
	...
	if (isUninitialized) {
		console.log(`ℹ️  Target database ${target.toUpperCase()} is uninitialized (0 remote migrations). All ${localVersions.length} local migrations are pending.`);
	}

	if (parity.pendingLocal.length > 0) {
		console.log(`⚠️  Pending local migrations not applied to remote (${parity.pendingLocal.length}):`);
	...
	if (parity.extraRemote.length > 0) {
		console.error(`❌ ERROR: Remote database has migrations not found in local workspace (${parity.extraRemote.length}):`);
	...
	if (parity.isReordered) {
		console.error(`❌ ERROR: Remote migration execution order is reordered or out of sequence.`);
	}

	if (parity.hasDivergentHistory) {
		console.error(`❌ ERROR: Remote migration history sequence diverges from expected local sequence.`);
	}
	...
	if (parity.isAligned) {
		console.log(`✅ ${target.toUpperCase()} database migration history is 100% aligned with local workspace.`);
	}
...
function checkTables(
	...
	for (const t of prodTables) {
		if (!localTableNames.has(t.tableName)) {
			console.error(`❌ ERROR: Table "${t.tableName}" exists in target but is missing locally!`);
			errors++;
		}
	}
	for (const t of localTables) {
		if (!prodTableNames.has(t.tableName)) {
			if (target === 'production' || target === 'preview') {
				console.log(`   INFO: Table "${t.tableName}" is local-only (expected addition before release).`);
			} else {
...
function checkColumns(
	...
	for (const col of prodCols) {
		...
		if (col.dataType !== localCol.dataType) {
			console.error(`❌ ERROR: Column "${key}" type mismatch! Target="${col.dataType}", Local="${localCol.dataType}"`);
			errors++;
		}
	}
	...
	for (const col of localCols) {
		...
		if (!prodCol && prodTableNames.has(col.tableName)) {
			if (target === 'production' || target === 'preview') {
				console.log(`   INFO: Column "${key}" is local-only (expected addition before release).`);
			} else {
...
function checkConstraints(
	...
	for (const c of prodConstraints) {
		if (!localConstraintNames.has(c.constraintName)) {
			console.error(`❌ ERROR: Constraint "${c.constraintName}" on "${c.tableName}" is missing locally!`);
			errors++;
		}
	}
	return errors;
...
function checkPolicies(
	...
	for (const p of prodPolicies) {
		const match = localPolicies.find((lp) => lp.tableName === p.tableName && lp.policyName === p.policyName);
		if (!match) {
			console.error(`❌ ERROR: RLS Policy "${p.policyName}" on "${p.tableName}" is missing locally!`);
			errors++;
		} else {
			...
			if (normProdQual !== normLocalQual) {
				console.error(`❌ ERROR: RLS Policy "${p.policyName}" on "${p.tableName}" mismatch in target vs local!`);
				errors++;
			}
		}
	}
...
function runSchemaAudit(target: string, dbUrl: string, initialErrors: number): number {
	...
	const targetFingerprint = generateFingerprint({
		...
		indexes: prod.indexes.map((i) => ({ ...i, indexDef: normalizeDef(i.indexDef, i.indexName) })),
		policies: prod.policies.map((p) => ({ ...p, qual: normalizeDef(p.qual || '', p.policyName), withCheck: normalizeDef(p.withCheck || '', p.policyName) })),
	...
	if (localReachable) {
		...
		const localFingerprint = generateFingerprint({
			...
			indexes: local.indexes.map((i) => ({ ...i, indexDef: normalizeDef(i.indexDef, i.indexName) })),
			policies: local.policies.map((p) => ({ ...p, qual: normalizeDef(p.qual || '', p.policyName), withCheck: normalizeDef(p.withCheck || '', p.policyName) })),
		...
		console.log('ℹ️  Local disposable database is not running; skipping detailed schema comparison.');
	}
