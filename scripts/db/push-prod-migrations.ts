...
async function main(): Promise<void> {
	...
	const guardResult = runCommand(
		'npx',
		['tsx', 'scripts/db/db-guard.ts', 'check', '--target', 'production', '--operation', 'migrate'],
		{ throwOnError: false }
	);
	...
	const dryRunResult = runCommand('supabase', ['db', 'push', '--db-url', prodDbUrl, '--dry-run'], {
		redact: [prodDbUrl],
	});
	...
	if (dryRunVersions.length === 0) {
		...
		for (const v of expectedVersions) {
			if (!dryRunSet.has(v)) {
				console.error(`❌ ERROR: Expected migration "${v}" is not in the dry-run list of pending migrations.`);
				match = false;
			}
		}
		for (const v of dryRunVersions) {
			if (!expectedSet.has(v)) {
				console.error(`❌ ERROR: Dry-run pending migration "${v}" is not in your explicit allowlist.`);
				match = false;
			}
		}

		if (!match) {
			fail('Migration dry-run does not match the explicit expected migrations allowlist. Aborting.');
		}
	...
	for (const v of expectedVersions) {
		if (!remoteVersions.includes(v)) {
			console.error(`❌ ERROR: Post-migration check failed: version "${v}" is not in remote schema_migrations!`);
			allApplied = false;
		}
	}

	if (!allApplied) {
		fail('Post-migration verification failed. Some allowlisted migrations were not recorded in remote.');
	}
