...
Implemented focused visual, leitmotif, hierarchy, and senior-friendly usability refinements for local invitation `/cumple/alba-rosa-quinones` only. The shared Location renderer now retains an eyebrow-only header and uses the concise `Google Maps` label; no presets or unrelated invitation profiles were modified.

| Dimension | Implemented Final State & Verification Results |
| --- | --- |
| **`70 AÑOS` Leitmotif** | Established `70 AÑOS` as an intentional visual leitmotif in: (1) Closed Envelope (`Alba Rosa` → `70 AÑOS`), (2) Letter (`70 AÑOS` → `ALBA ROSA QUIÑONES LÓPEZ`), (3) Hero (`70 AÑOS` eyebrow above headline), and (4) Location section intro eyebrow (`70 AÑOS · 11 DE SEPTIEMBRE DE 2026`). Perceived clearly without repetitive body copy. |
| **Reveal Hierarchy (Closed)** | Closed envelope presents seal monogram (`A·R`) → given name (`Alba Rosa`) → occasion (`70 AÑOS`) → date (`11 · SEP · 2026`). The envelope is enlarged within a height-aware constraint; the occasion, date, CTA, and guest label all meet the 14px practical metadata floor with moderated tracking. Personalized guest information sits outside the envelope before the sole external `ABRIR LA INVITACIÓN` action; a simulated long name wraps to two lines at 320×568 without overflow and leaves immediately as opening begins. |
| **Reveal Hierarchy (Letter)** | The letter contains only occasion (`70 AÑOS`) → honoree (`ALBA ROSA QUIÑONES LÓPEZ`) → date (`11 · SEP · 2026`). It has no guest block or second opening CTA. Letter paper is opaque ivory (`#F6F4F0`) with charcoal typography (`#2D2D2B`) and satin-champagne accents. |
| **Hero `70 AÑOS` Treatment** | Eyebrow `70 AÑOS` refined with uppercase `font-size: clamp(0.85rem, 2.2vw, 1.02rem)`, semibold weight (`600`), tracking (`0.24em`), satin champagne color (`#C9B894`), and text shadow (`0 2px 12px rgba(0,0,0,0.55)`). Spacing above headline set to `0.85rem`. `Alba Rosa Quiñones López` remains dominant headline. |
