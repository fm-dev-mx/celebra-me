...
| sectionOrder          | hero, quote, countdown, location, family, gallery, personalizedAccess, rsvp, thankYou                | verified       | owner OD2 + Goal 1 | Family restored with guest soft placeholders (“Por confirmar”); no padrinos/music/gifts/interludes; gallery = D4 only |
...
- Parent name mapping (`[[PENDIENTE:BRIDE_PARENTS]]`, `[[PENDIENTE:GROOM_PARENTS]]`) — Family
  publishes soft guest placeholders (`Por confirmar`) until resolved
...
### OD2 — section order (accepted; Goal 1 restores family)

1. `hero` (D2) → 2. `quote` → 3. `countdown` → 4. `location` → 5. `family` (soft placeholders) → 6.
   `gallery` (D4 only) → 7. `personalizedAccess` → 8. `rsvp` → 9. `thankYou` (no third photo)
...
| bucket                  | section keys                                                                          |
| ----------------------- | ------------------------------------------------------------------------------------- |
| accepted (OD2 + Goal 1) | hero, quote, countdown, location, family, gallery, personalizedAccess, rsvp, thankYou |
...
- Parents: placeholder tokens in this prep doc only (OD5). **Family section is published** with
  guest soft-copy `"Por confirmar"` until names are confirmed (no invented names; no CSS hide).
