...
| sectionOrder         | hero, quote, countdown, location, family, gallery, personalizedAccess, rsvp, thankYou          | verified                  | owner OD2           | Confirmed 2026-07-31; no padrinos/music/gifts/interludes; gallery = D4 only                                                           |
...
- Parent name mapping (`[[PENDIENTE:BRIDE_PARENTS]]`, `[[PENDIENTE:GROOM_PARENTS]]`)
...
1. `hero` (D2) → 2. `quote` → 3. `countdown` → 4. `location` → 5. `family` (placeholders; no padrinos) → 6. `gallery` (D4 only) → 7. `personalizedAccess` → 8. `rsvp` → 9. `thankYou` (no third photo)

**Omitted:** padrinos, music, gifts, interludes, multi-image gallery.
...
| bucket | section keys |
| ------ | ------------ |
| accepted (OD2) | hero, quote, countdown, location, family, gallery, personalizedAccess, rsvp, thankYou |
| omitted | padrinos/godparents, music, gifts, interludes |
| unresolved | — |
...
Owner constraint: **only these two files** (session 2026-07-31). WhatsApp export does **not** state a
two-photo limit; it anticipated digital session delivery and WeTransfer.
...
intentional multi-role. Guest-facing family uses “Por confirmar”; prep tokens remain below.
---

## Implementation Constraints

- prepReadiness is `READY_WITH_PLACEHOLDERS` — implementation allowed with documented parent placeholders (OD5); not `READY_FOR_IMPLEMENTATION` until parents resolve (optional) / no other blockers remain for full freeze.
- Lane A inheritance resets (when implementing): clear preset palette/copy that conflicts with olive–beige–gold and dual-venue itinerary; strip demo godparents.
- Lane B: none unless jewelry-box-wedding demo also needs the same palette (unlikely for one client).
- Padrinos: omit (`NO PONER`).
- Parents: placeholder tokens in this prep doc only (OD5). **Published** family shows “Por confirmar”
  (no raw `[[PENDIENTE:…]]` for guests; no invented names).
...
- Audit remediations 2026-07-31: see `.agent/plans/active/boda-perla-y-carlos-audit-backlog.md`.
